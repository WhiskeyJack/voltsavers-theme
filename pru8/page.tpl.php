...
<div id="header-wrapper">
	...
	<?php if (!$is_admin): ?>
		<div id="authorize">
      		    <ul><?php global $user; if ($user->uid != 0 && $user->name == "demo") { print '<li class="first">' .t('Logged in as ') .$user->name. '</li>'; print '<li><a href="' .url('user/logout'). '">' .t('Logout'). '</a></li>'; } elseif ($user->uid != 0) { print '<li class="first">' .t('Logged in as '). '<a href="' .url('user/'.$user->uid). '">' .$user->name. '</a></li>'; print '<li><a href="' .url('user/logout'). '">' .t('Logout'). '</a></li>'; } else { print '<li class="first"><a href="' .url('user'). '">' .t('Login'). '</a></li>'; } ?></ul>
		</div>
	<?php endif; ?>
